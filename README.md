...
Приложение использует оригинальный Go-клиент MasterDnsVPN в iOS-приложение, поднимает локальный SOCKS5-прокси на `127.0.0.1:41080` и держит туннель активным, пока вы переключаетесь в другое приложение (Shadowrocket, Happ и т.п.), которое уже пропускает через этот прокси системный трафик.
